...
# ==============================================================================
anos = [2024] #Troca para 2025
ondas = [6, 10, 16] # Desmarca as ondas que não tem interesse
...
def jucker(ds, onda, dia_alvo):
        ...
        data_inicio = pd.Timestamp(year=dia_alvo.year, month=1, day=1)
        data_fim = pd.Timestamp(year=dia_alvo.year, month=3, day=30)

        # ==============================================================================
        # 2. CÁLCULO DA MAGNITUDE MÁXIMA GLOBAL PARA ESCALONAMENTO FIXO (JUCKER 2021)
        # ==============================================================================
        idx_periodo = np.where((times >= data_inicio) & (times <= data_fim))[0]
        ...
        F_phi_periodo = ds["F_phi"].isel(z=idx_z, time=idx_periodo).transpose("lat", "z", "time").values
        F_z_periodo   = ds["F_z"].isel(z=idx_z, time=idx_periodo).transpose("lat", "z", "time").values
        ...
        idx_dia = np.where(times == dia_alvo)[0][0]
        ...
        F_z_raw   = ds["F_z"].isel(
            z=idx_z, time = slice(start, final)
            ).mean(dim="time").transpose("lat", "z").values
        ...
        logging.info(
            f"Dia {dia_alvo.strftime('%Y-%m-%d')} | Min accel: {minimu_f:.2f}, Max accel: {maximu_f:.2f}"
        )
        ...
        if minimu_f <= (-limiar) or maximu_f >= limiar:
            ...
            ax.set_title(
                f"Fluxo EP Q{onda}DW - {dia_alvo.strftime('%Y-%m-%d')}",
                fontsize=16,
            )

            ax.set_xlim(lat_min_lim, lat_max_lim)
            ax.set_ylim(z_min_lim, z_max_lim)
            ax.set_xticks(np.arange(lat_min_lim, lat_max_lim + 1, 10))
            ax.set_yticks(np.arange(z_min_lim, z_max_lim + 1, 10))

            # --- Escalonamento Físico e Aspect Ratio ---
            ...
            # --- Renderização 2D dos Vetores dos Dados ---
            ax.quiver(
                grid_lats_flat,
                grid_zs_flat,
                U_plot,
            ...
            # --- Vetor de Referência (Legenda) ---
            lat_ref_pos = lat_min_lim + 8.0
            z_ref_pos = z_max_lim - 5.0

            u_ref_phys = mag_ref_legenda
            u_ref_plot = (
                u_ref_phys * (X_Y / ((lat_max_lim - lat_min_lim) * np.pi / 180.0))
...
nc_path = 'D:\\database\\JAWARA\\ep_divs_q6.nc'

ds = xr.open_dataset(nc_path)
dia_alvo = dt.datetime(2025, 2, 19)
jucker(ds,  6, dia_alvo) 